...
/*
 * 0x8005 is the normal (MSB-first) representation expected by fsl_crc.
 * With input/output reflection enabled it implements the reflected 0xA001
 * recurrence used by CRC-16/MODBUS.
 */
static const crc_config_t s_crc16ModbusConfig = {
    .polynomial = 0x8005U,
    .seed = CRC16_MODBUS_INITIAL_VALUE,
    .reflectIn = true,
    .reflectOut = true,
...
/* Reflected 0xA001 table kept in flash: one lookup per input byte. */
...
static volatile bool s_crcHardwareBusy;

static bool Crc16Driver_ValidateArguments(const uint8_t *data, size_t length,
                                          const uint16_t *result)
{
    return (result != NULL) && ((data != NULL) || (length == 0U)) &&
           (length <= CRC16_MODBUS_MAX_DATA_SIZE);
}

static bool Crc16Driver_TryAcquireHardware(void)
{
    const uint32_t irqMask = DisableGlobalIRQ();
    const bool acquired = !s_crcHardwareBusy;

    if (acquired)
    {
        s_crcHardwareBusy = true;
    }
    EnableGlobalIRQ(irqMask);
    return acquired;
}

static void Crc16Driver_ReleaseHardware(void)
{
    const uint32_t irqMask = DisableGlobalIRQ();

    s_crcHardwareBusy = false;
    EnableGlobalIRQ(irqMask);
}

static uint16_t Crc16Driver_CalculateSoftwareUnchecked(const uint8_t *data, size_t length)
{
    ...
    {
        const uint8_t tableIndex = (uint8_t)(crc ^ data[index]);
        crc = (uint16_t)((crc >> 8U) ^ s_crc16ModbusTable[tableIndex]);
    }
    return crc;
}
...
void Crc16Driver_Init(void)
{
    ...
    s_crcHardwareBusy = true;
    CRC_Init(CRC0_PERIPHERAL, &s_crc16ModbusConfig);
    s_crcHardwareBusy = false;
    EnableGlobalIRQ(irqMask);
}
...
{
    if (!Crc16Driver_ValidateArguments(data, length, result))
    {
        return false;
    }
    ...
    if (Crc16Driver_TryAcquireHardware())
    {
        ...
        if (length != 0U)
        {
            CRC_WriteData(CRC0_PERIPHERAL, data, length);
        }
        *result = CRC_Get16bitResult(CRC0_PERIPHERAL);
        Crc16Driver_ReleaseHardware();
    }
    else
    {
        /* Never wait for CRC0: this table path is re-entrant and deterministic. */
        *result = Crc16Driver_CalculateSoftwareUnchecked(data, length);
    }
    return true;
}
...
bool Crc16Driver_VerifyFrame(const uint8_t *frame, size_t frameLength)
{
    ...
    if ((frame == NULL) || (frameLength < 2U) ||
        (frameLength > (CRC16_MODBUS_MAX_DATA_SIZE + 2U)))
    {
        return false;
    }
    if (!Crc16Driver_Calculate(frame, frameLength - 2U, &calculated))
    {
        return false;
    }

    received = (uint16_t)frame[frameLength - 2U] |
...
bool Crc16Driver_SelfTest(void)
{
    ...
    uint16_t automaticResult;
    uint16_t softwareResult;

    return Crc16Driver_Calculate(checkData, sizeof(checkData) - 1U, &automaticResult) &&
           Crc16Driver_CalculateSoftware(checkData, sizeof(checkData) - 1U, &softwareResult) &&
           (automaticResult == CRC16_MODBUS_CHECK_VALUE) &&
           (softwareResult == CRC16_MODBUS_CHECK_VALUE);
}
